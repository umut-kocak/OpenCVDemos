...
class BaseVideoDemo(BaseDemo):
    ...
    class FrameStatus(Enum):
        """
        Enum showing different actions to take depending on the frame status.
        """
        SUCCESS = 0
        IGNORE = 1
        ABORT = 2
    ...
    def __init__(self):
        ...
        if self.settings.video.capture.source != 0:
            ...
            if source_file:
                ...
                logger.error("Could not find the video source %s. Using camera instead.",
                  self.settings.video.capture.source)
    ...
    def check_captured_frame(self, frame):
        ...
        if frame is None:
            ...
            if self._number_of_null_frames > nr_tolerate > 0:
        ...
        while True:
            ...
            if frame_status:
                if frame_status == self.FrameStatus.ABORT:
                    ...
                    break
                if frame_status == self.FrameStatus.IGNORE :
                    continue

            # Processing the frame
            self._frame_timer.begin_label("PocessT(ms)")
            if self.settings.frame.processing.resize_capture_before_process:
                frame.image = resize_image(frame.image, self.settings.frame.processing.width,
                  self.settings.frame.processing.height)
    ...
    def register_keys(self):
        ...
        key_bindings = [
            ...
            (ord('D'), "Toggle VisualDebugger mode: Override or Separate.", lambda dbg: dbg.toggle_mode(),
              self._visual_debugger),
            (ord('+'), "Override next VisualDebugger image.",
              lambda dbg: dbg.isEnabled() and dbg.override_next(), self._visual_debugger),
            (ord('-'), "Override previous VisualDebugger image.",
              lambda dbg: dbg.isEnabled() and dbg.override_previous(), self._visual_debugger),

            # Video Manager
            (ord('V'), "Toggle VideoCapture mode: Single or multithreaded.", lambda vm: vm.toggle_strategy(),
              self._video_manager),
            (ord('f'), "Change the frame filter method of video capture strategy.",
            lambda vm: vm.capture_strategy.set_frame_filtering_method(vm.capture_strategy.frame_filtering.next()),
              self._video_manager),

            # Frame settings
            (ord('u'), "Increase waiting time at the end of the frame.", lambda s: adjust_wait_time(s, +1),
              self.settings.frame),
            (ord('j'), "Decrease waiting time at the end of the frame.", lambda s: adjust_wait_time(s, -1),
              self.settings.frame),

            # Saving and settings
            (ord('C'), "Save the current settings.",
              lambda s: s.save_to_json(self.get_output_folder()/"CurrentSettings.json"), self.settings),

            # Video capture skip number
            (ord('i'), "Increase video_capture_frame_filter_skip_number.", lambda s: adjust_skip_period(s, +1),
              self.settings.video.capture.frame_filtering.skip_frame),
            (ord('k'), "Decrease video_capture_frame_filter_skip_number.", lambda s: adjust_skip_period(s, -1),
              self.settings.video.capture.frame_filtering.skip_frame),
        ]
